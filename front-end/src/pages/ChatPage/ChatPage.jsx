...
// Call modal is globally provided by CallProvider
...
const ChatPage = () => {
	...
	useEffect(() => {
		...
		// Call events đã được xử lý ở CallProvider (global)
		...
		const loadFriends = async () => {
			...
			try {
				...
				if (response && response.errCode === 0) {
					...
					const formattedConversationsPromises = acceptedFriends.map(
						async (friend) => {
							...
							try {
								...
								console.error(
									`Error loading messages for friend ${friend.id}:`,
									err
								);
							}
					...
					const formattedConversations = await Promise.all(
						formattedConversationsPromises
					);
	...
	// Calling logic handled globally via CallContext
		...
		try {
			...
			if (response && response.errCode === 0) {
				...
				if (socket && socket.connected) {
					console.log(
						"Emitting editMessage socket event to recipient:",
						activeConversation.userId
					);
	...
	return (
		<>
						...
						<div className="chat-conversation">
							...
							<div className="chat-conversation-header">
								...
								<div className="conversation-info-main">
									<h3 className="conversation-name-main">
										{activeConversation.name}
									</h3>
									<p className="conversation-status">
										{activeConversation.isOnline
											? "Active now"
											: `Last seen ${activeConversation.lastMessageTime} ago`}
									</p>
								</div>
								<div className="chat-options">
									<button
										...
										onClick={() =>
											activeConversation &&
											startCall(activeConversation.userId, "audio", {
									...
									<button
										...
										onClick={() =>
											activeConversation &&
											startCall(activeConversation.userId, "video", {
												id: activeConversation.userId,
												name: activeConversation.name,
												avatar: activeConversation.avatar,
											})
							...
							{/* Messages (single source, with date dividers & edit) */}
			...
			{/* CallModal now rendered globally by CallProvider */}
		</>
	);
};
